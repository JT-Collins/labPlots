...
You will need to ensure you have the **Gotham Book** font installed on your computer. This font is used in the lab Power Point and Poster templates as well as the plots for consistency. 

Depending on the plot you may want/need to change the colours used, however, as a default we will use the UofL colour palette.
...
multiple_line_df <- gapminder |>
  filter(country == "Italy" | country == "United States" | country == "Afghanistan" ) 
...
#Make plot
bars <- ggplot(bar_df, aes(x = country, y = lifeExp, fill=country)) +
...
bars <- ggplot(bar_df, aes(x = country, y = lifeExp)) + # NOTE: removed fill from aes
  geom_bar(stat="identity", 
           position="identity",
           fill="#1380A1" # specified what colour we want
           ) +
  lab_style() +
